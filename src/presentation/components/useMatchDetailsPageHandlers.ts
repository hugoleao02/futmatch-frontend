...
import type { PartidaDetalhes } from '@domain/entities/PartidaDetalhes';
...
export function useMatchDetailsPageHandlers() {
  ...
  function handleEditarPartida() {
    ...
    if (partida && partida.id) {
      navigate(ROUTES.EDIT_MATCH.replace(':id', partida.id));
    }
  }
  ...
  const handleAceitarSolicitacao = async (solicitacaoId: string) => {
    ...
    try {
      ...
      if (solicitacaoAceita) {
        setPartida((prev: PartidaDetalhes | null) => {
          if (!prev) return null;
          return {
            ...prev,
            solicitacoes: prev.solicitacoes?.filter((req: User) => req.id !== solicitacaoId) || [],
            participantes: [
              ...(prev.participantes || []),
              {
                id: solicitacaoAceita.id,
                nome: solicitacaoAceita.nome,
                avatar: solicitacaoAceita.avatar,
                email: solicitacaoAceita.email,
              },
            ],
            participantesConfirmados: (prev.participantesConfirmados || 0) + 1,
          };
  ...
  const handleSortearTimes = async (numTimes: number) => {
    ...
    try {
      const response = await api.post<Team[]>(`/partidas/${matchId}/sortear-times`, { numTimes });
      setPartida(prev => {
        if (!prev) return null;
        return {
          ...prev,
