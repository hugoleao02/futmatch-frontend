...
export class UserAdapter {
   ...
   */
  static toUser(jogador: JogadorDTO): User {
    return {
      id: jogador.id,
   ...
   */
  static toJogadorDTO(user: User): JogadorDTO {
    return {
      id: user.id,
  ...
  static fromApiResponse(response: any): User {
    ...
    if (response && response.id !== undefined) {
      return this.toUser(response as JogadorDTO);
    }

    // Se a resposta tiver um campo jogador
    if (response && response.jogador) {
      return this.toUser(response.jogador as JogadorDTO);
    }
